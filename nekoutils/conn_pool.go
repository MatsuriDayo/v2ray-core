...
func (c *mangedFusedConn) WriteTo(p []byte, d net.Addr) (int, error) {
	if c2, ok := c.Conn.(net.PacketConn); ok {
		return c2.WriteTo(p, d)
	}
	return 0, errors.New("Not a PacketConn")
}

func (c *mangedFusedConn) ReadFrom(p []byte) (int, net.Addr, error) {
	if c2, ok := c.Conn.(net.PacketConn); ok {
		return c2.ReadFrom(p)
	}
	return 0, nil, errors.New("Not a PacketConn")
}
