...
func (s *DNS) lookupIPInternal(_domain dns.MatsuriDomainString, _option dns.IPOption) ([]net.IP, error) {
	...
	if a, ok := _domain.(*dns.MatsuriDomainStringEx); ok {
		...
		if ex.OptSkipFakeDNS {
			option.FakeEnable = false
		}
	...
	// Name servers lookup
	errs := []error{}
	for _, client := range s.sortClients(domain, ex, option) {
...
func (s *DNS) sortClients(domain string, ex *dns.MatsuriDomainStringEx, option dns.IPOption) []*Client {
	...
	if ex != nil && ex.OptInbound != nil {
		...
		for clientIdx, client := range s.clients {
			...
			if nekoutils.In(client.uidList.Uid, uid) {
				...
				clients = append(clients, client)
				clientIdxs = append(clientIdxs, clientIdx)
				hasMatch = true
			}
		}
	}
